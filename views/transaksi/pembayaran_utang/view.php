			...
			<tr>
				...
				<td>
					<?php if(!is_null($model->file)) { ?>
						<?= $this->action->link('view', $this->route->name('transaksi.pembayaran_utang.download_file', array('id' => $model->id)), 'class="btn btn-xs btn-primary"', $this->localization->lang('download_file')) ?>
					<?php } ?>
				</td>
			</tr>
			<tr>
		...
		<?php if ($model->batal) { ?>
			<table width="100%" class="table table-profile">
				<tr>
					...
					<td><?= $this->pembayaran_utang_m->enum('jenis_batal', $model->jenis_batal) ?></td>
				</tr>
				<tr>
